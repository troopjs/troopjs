	...
	grunt.registerTask("jsduck", "Build API Documentation with JSDuck.",function () {
		...
		function generate_config_files() {
			...
			(function guides_json() {
				...
				grunt.file.write(GUIDES_FILE, [HEADER, JSON.stringify([root], null, 2)].join('\n'));
			})();
			...
			(function config_json() {
				var config = {
					"--title": "TroopJS API Documentation",
					"--": [],
					"--exclude": [],
					"--warnings": [
						"-nodoc(class,public)", "-nodoc(member,public)"
					],
					"--external": [
						"Promise"
					],
				...
				var troop_modules = grunt.file.expand(
					'bower_components/troopjs-*'
				);

				config["--"] = troop_modules;
				// Excludes non-source files from JSDuck run.
				config["--exclude"] = _.reduce(troop_modules, function(excludes, module_path) {
					// Ignore tests.
					excludes.push(path.join(module_path, "test"));
					return excludes;
				}, []);
